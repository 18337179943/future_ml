...
class ModelStatistics:
    ...
    def caculate_statistics_total(self, df_li, res_save_pa='', prefix=''):
        '''计算全品种统计结果:
        '''
        res_dic_li = []
        for df in df_li:
...
class ConcatStatistics:
    ...
    def caculate_holding_rate(self, pa, is_tvt=0):
        ...
        if is_tvt:
            ...
            return func(train_pa), func(val_pa), func(test_pa)
        else:
            return func(pa)
    ...
    def trend_shock_score(self, symbol):
        ...
        if len(rate_res) != 0:
            ...
            df_res = df_res[(df_res['train_trend']<1.0) & (df_res['train_shock']<1.0)]
            # df_res = df_res.replace([-10], [None]).dropna()
    ...
    def concat_table(self, pa=None):
        ...
        for i in pa_li:
            ...
            for j in range(len(df_i)):  # 获取年化收益，把收益太高或者小于-0.04的去掉
                ...
                # is_select = 0 if (max_rate > 0.7 or min_rate < -0.01) else 1    # 过滤过拟合和欠拟合的模型
                is_select = 1
...
def run_concatstatistics(pa='', save_pa=''):
    ...
    if len(pa) == 0:
        ...
        pa = f'{pa_prefix}/datas/ml_result/model_2.0/original_max_ddpercent_duration_10_2'
        ...
        # pa = f'{pa_prefix}/datas/ml_result/model_2.0/factor_sort_sum_values_4'
        ...
        # save_pa = makedir(f'{pa_prefix}/datas/ml_result/model_2.0/original_max_ddpercent_2/model_zigzag')
        save_pa = f'{pa_prefix}/datas/ml_result/model_2.0/original_max_ddpercent_duration_10_2/model_raw_37'
        # save_pa = f'{pa_prefix}/datas/ml_result/model_1.0/model_zigzag'
