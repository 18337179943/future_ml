...
import random
from backtesting.ml_strategy_adj import MLStrategy, ArrayManager_m
# from simulation.base_strategy import BaseStrategy, ArrayManager_m
import talib as tb
sys.path.append("..")


class SimulationStrategy(MLStrategy):
    """
    trade_type
    0、前n根最高最低价止盈止损
    1、前n根涨跌幅超过k%, 当前只做反转
...
#         self.df_ma = df[df['symbol'] == self.symbol_name].copy()     # 获取品种主力合约数据
    ...
    def save_info(self, bar: BarData):
        ...
        if self.first:
            ...
            cost = one_hand_cost*self.hand if self.sig != 0 else 0
    ...
    def trade_mod0(self, bar: BarData):
        ...
        if bar.datetime.time() == time(11, 29) or bar.datetime.time() == time(10, 14) or bar.datetime.time() == time(22, 59) or bar.datetime.time() == time(23, 29) or \
            ...
            return  # model 1.0 修改
        # if bar.datetime.time() == time(4, 0): 
            # pass
        # if bar.datetime.time() == time(11, 29) or bar.datetime.time() == time(10, 14) or bar.datetime.time() == time(22, 59) or bar.datetime.time() == time(23, 29) or \
        #     bar.datetime.time() == time(0, 59) or bar.datetime.time() == time(2, 29) or bar.datetime.time() == time(14, 59):
        #     return  # bar.datetime.time() == time(9, 0) or bar.datetime.time() == time(21, 0) or bar.datetime.time() == time(11, 29) or 
        else:
            # self.hand = int(1_000_000 / bar.close_price / self.size)        # 1.0 版本没有的
            if self.pos == 0:
                if self.hand > 0:
                    if self.sig == 1:
                        self.buy(bar.close_price, self.hand)
                        # print(bar.datetime.time(), 'buy', bar.close_price)
                        # self.beign_trade = 1
                    elif self.sig == -1:
                        self.short(bar.close_price, self.hand)
                        # print(bar.datetime.time(), 'short', bar.close_price)
                        # self.beign_trade = -1
            elif self.pos > 0:
                if self.sig == -1:
                    self.sell(bar.close_price, abs(self.pos))
                    # print(bar.datetime.time(), 'sell', bar.close_price)
                    if self.hand > 0:
                        self.short(bar.close_price, self.hand)
                        # print(bar.datetime.time(), 'short', bar.close_price)
                    # self.beign_trade = -1
                ...
                if self.sig == 1:
                    ...
                    # print(bar.datetime.time(), 'cover', bar.close_price)
                    if self.hand > 0:
                        self.buy(bar.close_price, self.hand)
                        # print(bar.datetime.time(), 'buy', bar.close_price)
    ...
    def on_trade(self, trade: TradeData):
        ...
        # print('trade.datetime:', trade.datetime, 'trade.price:', trade.price)
        # input()
