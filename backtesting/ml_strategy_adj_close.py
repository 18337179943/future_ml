...
class MLStrategy(CtaTemplate):
    ...
    def save_info(self, bar: BarData):
        ...
        if self.first:
            self.first = 0
            signal = self.sig
            self.order_price = bar.close_price if self.sig != 0 else 0
            self.res_dic['profit'].append(0)
        else:
            signal = self.sig if self.sig == -np.sign(self.pos) else 0
            self.order_price = bar.close_price if self.sig == -np.sign(self.pos) else 0
            if trade_price == 0:
    ...
    def on_bar(self, bar: BarData):
        ...
        if self.pos == 0:
            if self.sig == 1:
                self.buy(self.order_price, self.hand)
                # self.beign_trade = 1
            elif self.sig == -1:
                self.short(self.order_price, self.hand)
                # self.beign_trade = -1
        elif self.pos > 0:
            if self.sig == -1:
                self.sell(self.order_price, abs(self.pos))
                self.short(self.order_price, self.hand)
                # self.beign_trade = -1
        elif self.pos < 0:
            if self.sig == 1:
                self.cover(self.order_price, abs(self.pos))
                self.buy(self.order_price, self.hand)
                # self.beign_trade = 1
