...
class SimulationStrategy(BaseStrategy):
    ...
    def on_init(self):
        ...
        # def filter_str(filter_str, li):
        #     '''从list里筛选出含有filtetr_str的字符串'''
        #     return list(filter(lambda x: filter_str in x, li))[0]

        self.df_contract = pd.read_csv(f'{pa_prefix}/datas/data_1m/{self.symbol_name}/{self.contract}.csv')
        self.model = joblib.load(self.y_pred)
        symbol_pa_li = os.listdir(self.load_pa)     # 品种结果相关文件路径
        index_pa = self.load_pa + filter_str('_adj', symbol_pa_li)     # 技术指标相关
        dm_series_pa = self.load_pa + filter_str('dm_series', symbol_pa_li)     # 极值参数
        dic_normal_pa = self.load_pa + filter_str('dic_normal', symbol_pa_li)   # 
        self.dm_series = joblib.load(dm_series_pa)    # dataframe
        self.dic_normal = joblib.load(dic_normal_pa)  # dic
        self.df_index_info = pd.read_csv(index_pa)   # 需要的指标信息
    ...
    def model_predict(self):
        ...
        df['open'], df['high'], df['low'], df['close'], df['volume'], df['turnover'] = \
            self.amn.open, self.amn.high, self.amn.low, self.amn.close, self.amn.volume, self.amn.turnover
