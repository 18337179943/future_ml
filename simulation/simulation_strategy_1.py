...
class SimulationStrategy(BaseStrategy):
    ...
    # trade_mod12
    init_balance = 1_000_000  # 1_000_000
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        if self.is_leverage == 0:
            self.hand = int(self.init_balance * self.hand * 0.15 / 300_000)
...
#         self.df_ma = df[df['symbol'] == self.symbol_name].copy()     # 获取品种主力合约数据
    ...
    def trade_mod0(self, bar: BarData):
        ...
        #     bar.datetime.time() == time(0, 59) or bar.datetime.time() == time(2, 29) or bar.datetime.time() == time(14, 59): # or bar.datetime.time() == time(14, 59)
        ...
        if bar.datetime.time() == time(14, 59) or bar.datetime.time() == time(21, 59) or bar.datetime.time() == time(22, 59) or bar.datetime.time() == time(23, 29) or \
            ...
            if self.pos * self.sig < 0 or (self.pos == 0 and self.sig != 0):
                self.hand = self.init_balance / bar.close_price / self.size
                self.open_hand = self.hand // 3
                if self.open_hand == 0: self.open_hand = 1
        #     return  # bar.datetime.time() == time(9, 0) or bar.datetime.time() == time(21, 0) or bar.datetime.time() == time(11, 29) or 
        # else:
        min_open_hand = min(abs(self.hand-abs(self.pos)), self.open_hand)
        min_close_hand = min(abs(self.pos), self.open_hand)
        if self.pos == 0:
            if self.hand > 0:
                if self.sig == 1:
                    self.buy(bar.close_price, min_open_hand)
                elif self.sig == -1:
                    self.short(bar.close_price, min_open_hand)
        elif self.pos > 0:
            if self.sig == -1:
                self.sell(bar.close_price, min_close_hand)
                # if self.hand > 0:
                #     self.short(bar.close_price, self.hand)
        elif self.pos < 0:
            if self.sig == 1:
                self.cover(bar.close_price, min_close_hand)
                # if self.hand > 0:
