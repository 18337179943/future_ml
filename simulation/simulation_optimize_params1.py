...
def run_optimize_total_mltest():
    ...
    print(val_score, test_score, 1, 4, 0.1, '-----------')
    
    # for i in range(0, 2, 1):
    #     for j in range(7, 16, 3):
    #         for k in range(2, 18, 2):
    #             # params = {"trade_type": 14, "pos_mod": i, "profit_rate_n": 30}
    #             # params = {"trade_type": 13, "small_loss_n": 4, 'big_loss_n': 2, 'big_rate_small': 2, 'profit_rate_n': 30}
    #             # params = {"trade_type": 10, "loss_n": i, 'is_win': 1, 'is_leverage': 0}
    #             # params = {"trade_type": 15, "atr_n_mod13": i, 'atr_dev_mod13': j/10}
    #             params = {"trade_type": 14, "limit_n": i, 'atr_n_mod14': j, 'atr_dev_mod14': k/10}
    #             val_score, _ = sbt.all_symbols_backtesting(symbol_li, startdate=datetime(2019, 5, 1), enddate=datetime(2020, 5, 1), y_pred_li=y_pred_li, params=params, target_type=target_type)
    #             test_score, _ = sbt.all_symbols_backtesting(symbol_li, startdate=datetime(2020, 5, 1), enddate=datetime(2020, 10, 31), y_pred_li=y_pred_li, params=params, target_type=target_type)
    #             print(val_score, test_score, i, j, k, '-----------')
