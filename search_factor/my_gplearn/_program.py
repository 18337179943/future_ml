...
class _Program(object):
    ...
    def build_program(self, random_state):
        ...
        program = [function]
        if function.type == 'ts':
            terminal_stack = [function.arity-1]
        else:
        ...
        while terminal_stack:
            ...
            if (depth < max_depth) and (method == 'full' or
                ...
                if function.type == 'ts':
                    terminal_stack.append(function.arity - 1)
                else:
                ...
                while terminal_stack[-1] == 0:
                    if function_stack[-1].type == 'ts':
                        window_list = [4, 6, 12, 24]
                        # window_list = [4, 6]
                        terminal = window_list[random_state.randint(len(window_list))]
                        program.append(terminal)
                        self.ts_arg_list.append(len(program)-1)
                    terminal_stack.pop()
                    function_stack.pop()
    ...
    def print_str(self, program, ts_arg_list):
                    ...
                    if self.feature_names is None:
                        ...
                        if self.ts_arg_list == []:
                            try:
                                ...
                                print(node, len(self.feature_names))
    ...
    def execute(self, X):
        ...
        for node in self.program:
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                if function.type == 'ts':
                    terminals = [
                        np.reshape(np.repeat(t, X.shape[0] * X.shape[1]), (X.shape[0], X.shape[1])).T if isinstance(t, float)
                        else X[:, :, t].T if isinstance(t, int)
                        else t for t in apply_stack[-1][1:-1]]
                    terminals.append(apply_stack[-1][-1])
                else:
                    terminals = [
                        np.reshape(np.repeat(t, X.shape[0] * X.shape[1]), (X.shape[0], X.shape[1])).T if isinstance(t, float)
                        else X[:, :, t].T if isinstance(t, int)
    ...
    def raw_fitness(self, X, y, max_samples, sample_weight1, sample_weight2):
        ...
        # print('done.')
        # input()
        # pd.DataFrame(y.T).to_csv('D:/策略开发/futures_ml/search_factor/my_gplearn/raw_data/y_test.csv', index=False)
        # pd.DataFrame(y_pred.T).to_csv('D:/策略开发/futures_ml/search_factor/my_gplearn/raw_data/y_pred.csv', index=False)
        # print('done2')
        # input()
        # sample_weight1, sample_weight2 = 1.0, 1.0

        if max_samples == y.shape[1]:
            mean_fitness1 = self.metric(y[:, :max_samples].copy(), y_pred[:, :max_samples].copy(), sample_weight1)
            mean_fitness2 = mean_fitness1
        else:
            mean_fitness1 = self.metric(y[:, :max_samples].copy(), y_pred[:, :max_samples].copy(), sample_weight1)
            mean_fitness2 = self.metric(y[:, max_samples:].copy(), y_pred[:, max_samples:].copy(), sample_weight2)

        metric_name = self.metric.function.__name__
        if metric_name == '_total_return_all_quantile':
            # print(self.metric.function.__name__)
            mean_fitness1, quantiles_20, quantiles_80 = mean_fitness1
            mean_fitness2, _, _ = mean_fitness2
        elif metric_name == '_rank_ic':
            mean_fitness1, val1, quantiles_20, quantiles_80 = mean_fitness1
            mean_fitness2, val2, _, _ = mean_fitness2
        elif metric_name == '_cumsum_return_rate':
            mean_fitness1 = mean_fitness1
            mean_fitness2 = mean_fitness2

        # np.save('sectional_pred\\{}_pred.npy'.format(hash(self.__str__())), temp2)
        if (mean_fitness2 + mean_fitness1) / 2 > self.save_thread:
            if metric_name == '_total_return_all_quantile':
                suffix = round((mean_fitness2 + mean_fitness1)/2,4)
                try:
                    np.save('D:/策略开发/futures_ml/search_factor/my_gplearn/good_total_return_all_quantile_normal_3_7030/' + str(suffix) + '_' + \
                    str(quantiles_20) + '_' + str(quantiles_80) + '_' + metric_name + '__' +self.__str__() + '.npy', np.nan)
                except:
                    pass
            elif metric_name == '_rank_ic':
                suffix = round((val1 + val2)/2,4)
                np.save('D:/策略开发/futures_ml/search_factor/my_gplearn/good/' + str(suffix) + '_' + \
                str(quantiles_20) + '_' + str(quantiles_80) + '_' + metric_name + '__' +self.__str__() + '.npy', np.nan)
            else:
                suffix = round((mean_fitness2 + mean_fitness1)/2,4)
                np.save('D:/策略开发/futures_ml/search_factor/my_gplearn/good/' + str(suffix) + '_' + metric_name + '__' +self.__str__() + '.npy', np.nan)
            
        print('in_sample_fitness:'+str(round(mean_fitness1,4)))
        print('out_sample_fitness:'+str(round(mean_fitness2,4)))
        print('--------------------------------------')

        # endtime = datetime.datetime.now()
        # print(endtime)

        return mean_fitness1, mean_fitness2
    ...
    def reproduce(self):
        ...
        return copy(self.program), self.ts_arg_list, self.__str__()
    ...
    def crossover(self, donor, random_state):
        ...
        while(start in self.ts_arg_list):
            start, end = self.get_subtree(random_state)

        removed = range(start, end)
        # Get a subtree to donate
        donor_start, donor_end = self.get_subtree(random_state, donor.program)
        while(donor_start in donor.ts_arg_list):
            donor_start, donor_end = self.get_subtree(random_state, donor.program)

        ts_arg_list1 = [i for i in self.ts_arg_list if i < start]
        ts_arg_list2 = [i for i in self.ts_arg_list if i >= end]
        ts_arg_list2 = [i + donor_end - donor_start - end + start for i in ts_arg_list2]
        donor_ts_arg_list = [i for i in donor.ts_arg_list if i >= donor_start and i < donor_end]
        donor_ts_arg_list = [i + start - donor_start for i in donor_ts_arg_list]

        ts_arg_list = ts_arg_list1 + donor_ts_arg_list + ts_arg_list2

        donor_removed = list(set(range(len(donor.program))) -
                             set(range(donor_start, donor_end)))

        #print('~~~~~~~~~~~~~~~~~~~~~')
        #print(self.print_str(self.program[:start] + donor.program[donor_start:donor_end] + self.program[end:],ts_arg_list ))
        #print(ts_arg_list)
        #print('-------------------')

        # Insert genetic material from donor
        return (self.program[:start] + donor.program[donor_start:donor_end] + self.program[end:]), removed, donor_removed, ts_arg_list, self.print_str(self.program[:start] + donor.program[donor_start:donor_end] + self.program[end:],ts_arg_list )

    def subtree_mutation(self,chicken, random_state):
    ...
    def point_mutation(self, random_state):
        ...
        for node in mutate:
            if node in self.ts_arg_list:
                replacement = random_state.randint(3, 10)
                ...
                if isinstance(program[node], _Function):
                    ...
                    while True:
                        ...
                        if replacement.type == program[node].type:
                            break
                    program[node] = replacement
                else:
        ...
        return program, list(mutate), self.ts_arg_list, self.print_str(program, self.ts_arg_list)
