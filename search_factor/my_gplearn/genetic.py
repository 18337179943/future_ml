...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params, save_thread):


    def create_program(function_set, arities, init_depth, init_method, n_features, metric, transformer, const_range,
                       p_point_replace, parsimony_coefficient, feature_names, random_state, program, ts_arg_list, save_thread):
    ...
    while i < n_programs:
        ...
        if parents is None:
            ...
            if method < method_probs[0]:
                ...
                while True:
                    ...
                    program, removed, remains, ts_arg_list, program_str  = parent.crossover(donor, random_state)
                    trial += 1
                    if program_str not in programs_str or trial > 10:
                ...
                trial = 0
                while True:
                    chicken = create_program(function_set, arities, init_depth, init_method, n_features, metric,
                                             transformer, const_range, p_point_replace, parsimony_coefficient,
                                             feature_names, random_state, None, [], save_thread=save_thread)
                    program, removed, remains, ts_arg_list, program_str = parent.subtree_mutation(chicken, random_state)
                    trial +=1
                    if program_str not in programs_str or trial > 10:
                        programs_str.append(program_str)
                        break
                ...
                trial = 0
                while True:
                    program, mutated, ts_arg_list, program_str = parent.point_mutation(random_state)
                    trial += 1
                    if program_str not in programs_str or trial > 10:
                        programs_str.append(program_str)
                        break
                ...
                # reproduction
                program, ts_arg_list, program_str = parent.reproduce()
                if program_str in programs_str:
                    continue
        ...
        program = create_program(function_set, arities, init_depth, init_method, n_features, metric, transformer, const_range, p_point_replace, parsimony_coefficient, feature_names, random_state, program, ts_arg_list, save_thread=save_thread)
        ...
        program.raw_fitness_, program.oob_fitness_ = program.raw_fitness(X, y, max_samples, curr_sample_weight, oob_sample_weight)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            print('    |{:^42}|{:^42}|'.format('Population Average',
                                               'Best Individual'))
            print('-' * 4 + ' ' + '-' * 25 + ' ' + '-' * 42 + ' ' + '-' * 10)
            line_format = '{:>4} {:>8} {:>16} {:>16} {:>8} {:>16} {:>16} {:>10}'
            print(line_format.format('Gen', 'Length', 'Fitness', 'OOB Fitness', 'Length',
                                     'Fitness', 'OOB Fitness', 'Time Left'))

            #f.write('    |{:^42}|{:^42}|\n'.format('Population Average', 'Best Individual'))
            #f.write('-' * 4 + ' ' + '-' * 25 + ' ' + '-' * 42 + ' ' + '-' * 10 + '\n')
            #f.write(line_format.format('Gen', 'Length', 'Fitness', 'OOB Fitness', 'Length', 'Fitness', 'OOB Fitness', 'Time Left'))
        else:
            ...
            average_oob_fitness = run_details['average_oob_fitness'][-1]
            oob_fitness = run_details['best_oob_fitness'][-1]
            line_format = '{:4d} {:8.2f} {:16g} {:16g} {:8d} {:16g} {:16g} {:>10}'
            
                    
            line = line_format.format(run_details['generation'][-1],
    ...
    def fit(self, X, y, sample_weight=None):
       ...
       '''
        ...
        if isinstance(self.metric, _Fitness):
            ...
            # if self.metric not in ('pearson', 'spearman', 'rank_ic', 'top_return', 'mutual_info', 'm_spearman', 'total_return'):
            #     raise ValueError('Unsupported metric: %s' % self.metric)
            self._metric = eval('make_fitness(function=_%s, greater_is_better=True)' % (self.metric))
        ...
        for gen in range(prior_generations, self.generations):
            ...
            population = _parallel_evolve(self.population_size,
                                          parents,
                                          X,
                                          y,
                                          sample_weight,
            ...
            oob_fitness = [program.oob_fitness_ for program in population]
            ...
            if self._metric.greater_is_better:
                best_program = population[np.argmax(oob_fitness)]
            else:
                best_program = population[np.argmin(oob_fitness)]

            self.run_details_['generation'].append(gen)
            self.run_details_['average_length'].append(np.mean(length))
            self.run_details_['average_fitness'].append(np.nanmean(fitness))
            self.run_details_['average_oob_fitness'].append(np.nanmean(oob_fitness))
            self.run_details_['best_length'].append(best_program.length_)
            self.run_details_['best_fitness'].append(best_program.raw_fitness_)
            best_oob_fitness = np.nan
            if self.max_samples < 1.0:
                best_oob_fitness = best_program.oob_fitness_
            self.run_details_['best_oob_fitness'].append(best_oob_fitness)
        ...
        if isinstance(self, TransformerMixin):
            ...
            ##################
            # evaluation = np.array([np.load('D:/策略开发/futures_ml/search_factor/my_gplearn/sectional_pred/{}_pred.npy'.format(hash(gp.__str__()))) for gp in
            ...
            correlations_3d = np.zeros([evaluation.shape[0],evaluation.shape[0],evaluation.shape[1]])

            for i in range(evaluation.shape[1]):
                with np.errstate(divide='ignore', invalid='ignore'):
                    temp = evaluation[:,i,:]
                    temp = np.delete(temp,np.where(np.isnan(temp))[1], axis = 1)
                    correlations_3d[:,:,i] = np.abs(np.corrcoef(temp))
            correlations = np.nanmean(correlations_3d, axis = 2)
            ...
            if self._metric.greater_is_better:
                self._program = self._programs[-1][np.argmax(oob_fitness)]
            else:
                self._program = self._programs[-1][np.argmin(oob_fitness)]
