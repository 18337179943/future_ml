...
class MLTest():
    ...
    7、对回测结果画图和保存
    '''
    def __init__(self, load_pa=f'{pa_prefix}/datas/ml_result/symbol_result_adj/params/') -> None:
        self.load_pa = load_pa
        self.symbol_res_pa_li = os.listdir(load_pa) # 获取每个品种的文件路径
        self.mainconinfo = MainconInfo()
        self.startdate, self.enddate = datetime(2016, 1, 1), datetime(2022, 8, 16)   # datetime(2016, 1, 1), datetime(2020, 11, 1) datetime(2020, 12, 1), datetime(2021, 12, 31)
        self.train_date, self.val_date, self.test_date = datetime(2016, 5, 1), datetime(2019, 5, 1), datetime(2020, 5, 1)
        ...
        # self.symbols_li = ['AP', 'C', 'FG', 'HC', 'CF', 'J', 'JD', 'JM', 'L', 'M', 'MA', 'OI', 'P', 'PP', 'RB', 'RM', 'RU', 'SF', 'SR', 'V', 'Y',
        #          'AG', 'CU', 'AL', 'ZN', 'PB', 'SN']

        # self.symbols_li = ['ZN', 'Y', 'AP']
        # self.symbols_li = ['CF', 'JD', 'ZN', 'OI', 'RU', 'FG', 'Y', 'MA']
    ...
    def generate_y_pred(self, symbol, interval='60', need_df_index=0):
        ...
        df_contracts = self.mainconinfo.get_symbol_df_maincon(symbol, 
                        self.startdate, self.enddate, delay=15)   # 获取主力合约列表和主力合约时间段
        ...
        for i in range(len(df_contracts)):      # 品种的合约
            ...
            if df['datetime'].iloc[0] > startdate:
                print('del ', contract)
                continue
