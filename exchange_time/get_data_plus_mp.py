...
def run_get_data_plus1(data_name):
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=len(data_name), mp_context=ctx) as executor:  # max_workers=10
        executor.map(run_get_data_plus1, data_name)
   
    # pool = multiprocessing.Pool(len(data_name))
    
    # for item in data_name:
    #     print(item)
    #     pool.apply_async(run_get_data_plus1(item))
    # pool.close()
    # pool.join()
