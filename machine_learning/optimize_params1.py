...
class OptimizeParams():
    ...
    def target_func(self, trial, study, train_data_set=None, prune=0):
        ...
        # 修改1.0
        df_importance = self.classifier.show_importance(is_save=0)
        if len(df_importance[df_importance['importance']>0]) < 3:
    ...
    def optuna_optimize(self, n_trials, prune=0):
        ...
        print("------------------------------------------------")
        acc_res, pa_predict = run_train(symbol=self.symbol, parameters=None, suffix=self.suffix, method=self.method)
...
def run_train_model(symbol, suffix, y_thread, index_n, method, target_type='drawdown', pa=None, x_thread_li=None):
    '''单品种跑训练，使用1.0版本的参数'''
    bm = BaseModel()
    index_n, _ = run_dp(symbol, suffix, y_thread, index_n, method)
    y_suffix = f'{y_thread}_{suffix}'
    bp = bm.get_model_params(suffix=y_suffix, is_save=1, pa=pa, x_thread_li=x_thread_li)
    acc_res, pa_predict = run_train(symbol=symbol, parameters=None, suffix=y_suffix, method=method, target_type=target_type)
    res_dic = {'symbol': [], 'best_params': []}
    res_dic['symbol'].append(symbol)
    res_dic['best_params'].append(bp)
    res_dic.update(acc_res)
    return res_dic, index_n, pa_predict
...
def run_dp_optimize_all(symbol, threadhold_li, y_thread_li, n_trials=300, is_optuna=1):
    '''全品种跑优化'''
    method = 0  # target方法
    # n_trials = 300    # 优化迭代步数
    ...
    target_type = 'drawdown'  # 'drawdown' max_ddpercent_duration
    ...
    param_pa = f'{pa_prefix}/datas/ml_result/model_2.0/original_same_value_10_2/params/'
    x_thread_li = threadhold_li
    makedir(pa_res)
    bt = BackTester()
    ...
    for y_thread in y_thread_li:
        for threadhold in threadhold_li:
            try:
                ...
                continue
            num_index = suffix.split('_')[-5]
            if eval(num_index) < 3:
                print(suffix, 'got less index.')
                continue
            ...
            if is_optuna:
                ...
                res_dic, index_n, pa_predict = run_train_model(symbol, suffix, y_thread, index_n, method, target_type=target_type, pa=param_pa, x_thread_li=x_thread_li)  # 跑训练
