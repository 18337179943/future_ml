...
class OptimizeParams():
    def __init__(self, symbol=None, suffix='', method=0, model_name='lightgbm') -> None:
        self.suffix = suffix
    ...
    def target_func(self, trial, study, train_data_set=None):
        ...
        acc_train, acc_val, acc_test = self.classifier.calculate_roc_auc_all(binary=1)
    ...
    def optuna_optimize(self, n_trials, binary=1):
        '''optuna调参'''
        # try:
        ...
        print("------------------------------------------------")
        acc_res = run_train(symbol=self.symbol, parameters=None, suffix=self.suffix, method=self.method, binary=binary)
...
def run_optuna(symbol, suffix, y_thread, index_n, method, n_trials, model_name, binary=1):
    index_n = run_dp(symbol, suffix, y_thread, index_n, method)
    s = OptimizeParams(symbol, f'{y_thread}_{suffix}', method, model_name)
    res_dic = s.optuna_optimize(n_trials, binary=binary)
...
def main():
    ...
    binary = 1
    del_interval = '_30m'
    save_interval = '_120m'  # y的周期
    model_name = 'lightgbm'   # 'lightgbm' 'catboost' 还没有完成
    index_n = 500   # 指标数量阈值 25
    y_thread = 0  # y的参数 [5, 0.5, 1, 0] [10, 1, 1, 0] 0.7 [周期，几倍标准差，均值, y方法0，1]
    ...
    # suffix = f'rb_60m_0.65_21_1_return_rate_360m'
    suffix = f'rb_60m_0.8_23_1_return_rate_120m'
    # suffix = f'{symbol}_{win_n}m_0.05_50_1_return_rate{save_interval}'
    print(suffix)
    res_dic, index_n = run_optuna(symbol, suffix, y_thread, index_n, method, n_trials, model_name, binary=binary)
